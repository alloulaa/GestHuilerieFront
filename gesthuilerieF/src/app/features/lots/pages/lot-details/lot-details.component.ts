...
export class LotDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.form = this.fb.group({
      ...
      dateRecolte: [''],
      dateReception: [''],
      fournisseurId: [null as number | null, [Validators.required, Validators.min(1)]],
  ...
  async saveLot(): Promise<void> {
    ...
    this.lotOlivesService.update(this.lot.idLot, {
      ...
      fournisseurId: Number(raw.fournisseurId ?? 0),
  ...
  private buildFormValue(lot: LotOlives): Record<string, unknown> {
    return {
      ...
      fournisseurId: Number(lot.fournisseurId ?? 0) || null,
